        ...
        <scroll ref="listContent" :refreshDelay="refreshDelay" class="list-content" :data="sequenceList">
          <transition-group name="list" tag="ul">
            <li ref="listItem" class="item" @click="selectItem(item,index)" v-for="(item, index) in sequenceList" :key="item.id">
              ...
              <span class="text">{{getDesc(item)}}</span>
...
<script type="text/ecmascript-6">
  ...
  export default{
    ...
    computed: {
      modeText() {
        return this.modes === playMode.sequence ? '顺序播放' : this.modes === playMode.random ? '随机播放' : '单曲循环';
      }
    },
    methods: {
      show() {
      ...
      getDesc(song) {
        return `${song.name} - ${song.singer} . ${song.album}`;
      },
      ...mapActions([
        'deleteSong',
        'deleteSongList'
